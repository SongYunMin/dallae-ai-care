        ...
        <div className="rounded-3xl bg-card shadow-card p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <StatusBox
              ...
              sub={lastFeeding ? formatRelative(lastFeeding.recordedAt) : ''}
              tone="cream"
            />
            <StatusBox
              label="마지막 낮잠"
              value={lastSleep ? '잠' : '기록 없음'}
              sub={lastSleep ? formatRelative(lastSleep.recordedAt) : ''}
              tone="sky"
            />
            <StatusBox
              label="기저귀"
              value={lastDiaper ? '정상' : '기록 없음'}
              sub={lastDiaper ? formatRelative(lastDiaper.recordedAt) : ''}
              tone="mint"
            />
            <StatusBox
              label="약 복용"
              value={lastMed ? '복용' : '기록 없음'}
              sub={lastMed ? formatRelative(lastMed.recordedAt) : '-'}
              tone="coral"
            />
          </div>
        </div>
