...
| 돌봄 모드 | `/care-mode` | 돌봄 세션 시작/종료, 빠른 기록, 음성 기록, 감사 메시지 생성 | `test_care_session_voice_note_end_and_thankyou_flow` |
...
| 가족/돌봄자 | `/family` | 초대 링크 생성, 권한 선택, 감사 메시지 기본값, 로그아웃 | `test_invite_flow_create_get_accept`, `npm run build` |
| 초대 수락 | `/invite/$token` | 초대 조회 및 수락 | `test_invite_flow_create_get_accept` |
...
| `POST /api/families/{family_id}/invites` | 돌봄자 초대 생성 | `test_invite_flow_create_get_accept` |
| `GET /api/invites/{token}` | 초대 링크 조회 | `test_invite_flow_create_get_accept` |
| `POST /api/invites/{token}/accept` | 초대 수락 및 멤버 생성 | `test_invite_flow_create_get_accept` |
...
| `POST /api/care-sessions/start` | 돌봄자 세션 시작 | `test_care_session_voice_note_end_and_thankyou_flow` |
